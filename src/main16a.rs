...
fn optimize_path<'a>(
    from: &str, // current location. Starts with AA
    ...
    let from_valve = valves_graph.get(from).unwrap();
    if remaining_time <= 2 {
    ...
    let max_value = remaining_destinations
        ...
        .map(|v| {
            ...
            let time_needed = *from_valve.path_lengths.get(*v).unwrap() as i32 + 1;
    ...
    for (i, dest) in remaining_destinations.iter().enumerate() {
        ...
        let Some(path) = from_valve.path_lengths.get(dest) else {
            continue;
        };
        // each one of the nodes in path takes 1 min, then 1 min to open the valve
        let rem_time = remaining_time - *path as i32 - 1;
        if rem_time < 0 {
            continue;
        }
        ...
        let Some(mut tail) = optimize_path(*dest, &valves_graph, new_remaining_destinations, next_alpha_factor, rem_time, value) else {
            continue;
        };
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let optimal_path = optimize_path("AA", &valves, destinations, 0, 30, 0);
